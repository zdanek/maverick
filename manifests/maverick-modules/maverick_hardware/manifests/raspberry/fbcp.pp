class maverick_hardware::raspberry::fbcp (
    ...
    oncevcsrepo { "git-raspi2fb":
        ...
        dest        => "/srv/maverick/var/build/raspi2fb",
    } ->
    file { "/srv/maverick/var/build/raspi2fb/build":
        ensure      => directory,
    } ->
    exec { "build-raspi2fb":
        timeout     => 0,
        command     => "/usr/bin/cmake .. && /usr/bin/make && /usr/bin/make install >/srv/maverick/var/log/var/build/raspi2fb.build.out 2>&1",
        cwd         => "/srv/maverick/var/build/raspi2fb/build",
